...
public class HdfsUtil {
    ...
    //搜索文件
    public List<String> findFile(Path path, String fileNamePattern) throws IOException {
        FileSystem fs = getFileSystem();
        ...
        if (fileStatuses != null) {
            for (FileStatus fileStatus : fileStatuses) {
                ...
                if (!fileStatus.isDirectory()) {
                    if (pattern.matcher(fileStatus.getPath().getName()).matches()) {
                        filesPaths.add(fileStatus.getPath().toString());
                        System.out.println("找到文件: " + fileStatus.getPath());
                    }
                } else {
                    // 如果是目录，递归搜索
                    filesPaths.addAll(findFile(fileStatus.getPath(), fileNamePattern));
                }
            }
        }
        return filesPaths;
